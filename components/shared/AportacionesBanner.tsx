...
export default function AportacionesBanner({ recurring, funds, onConfirm, operations }: Props) {
  const today    = new Date();
  const monthKey = `${String(today.getMonth() + 1).padStart(2,'0')}/${today.getFullYear()}`;
  const dayOk    = today.getDate() >= 10; // a partir del día 10
  ...
  const handleConfirm = (r: RecurringContribution) => {
    ...
    // Actualizar capital invertido del fondo
    ...
    // Marcar como registrada este mes
    const updatedR = { ...r, lastRegistered: monthKey };
    onConfirm(updatedR, newFunds, [newOp, ...operations]);
  };
    ...
    <div style={{ background: 'linear-gradient(135deg,#064e3b,#065f46)', border: '1px solid #10b98133', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1.25rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: '#6ee7b7' }}>Es día {today.getDate()} — tienes {pendientes.length} aportación{pendientes.length > 1 ? 'es' : ''} programada{pendientes.length > 1 ? 's' : ''} para este mes</div>
        </div>
      </div>
